local function get_turret_entity(pos)
	...
	for _, obj in ipairs(objects) do
		...
		if ent then
			if ent.name == "lucky_weapons:turret_entity" then
				-- Remove duplicates
				if entity then
					obj:remove()
				else
...
minetest.register_entity("lucky_weapons:tnt_entity", {
	physical = false,
	timer = 0,
	visual = "cube",
...
minetest.register_entity("lucky_weapons:turret_entity", {
	physical = true,
	visual = "mesh",
	...
	on_activate = function(self, staticdata)
		...
		if minetest.get_node(self.pos).name ~= "lucky_weapons:turret" then
			self.object:remove()
			return
		end
	...
	fire = function(self)
		...
		local obj = minetest.add_entity(pos, "lucky_weapons:tnt_entity")
		if obj then
			local ent = obj:get_luaentity()
			if ent then
...
minetest.register_node("lucky_weapons:turret", {
	...
	after_place_node = function(pos, placer)
		...
		if node.name == "air" then
			if not get_turret_entity(pos) then
				minetest.add_entity(pos, "lucky_weapons:turret_entity")
			end
		end
	end,
