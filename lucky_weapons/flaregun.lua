minetest.register_craftitem("lucky_weapons:flare", {
	description = "Flare",
	inventory_image = "shooter_flare_inv.png",
})

minetest.register_node("lucky_weapons:flare_light", {
	drawtype = "glasslike",
...
minetest.register_abm({
	nodenames = "lucky_weapons:flare_light",
	interval = 5,
	chance = 1,
...
minetest.register_entity("lucky_weapons:flare_entity", {
	physical = true,
	timer = 0,
	visual = "cube",
		...
		if self.timer > 0.2 then
			...
			if node.name ~= "air" then
				...
				if minetest.get_node(pos).name == "air" and
					...
					minetest.place_node(pos, {name="lucky_weapons:flare_light"})
					local meta = minetest.get_meta(pos)
...
minetest.register_tool("lucky_weapons:flaregun", {
	description = "Flare Gun",
	inventory_image = "shooter_flaregun.png",
	on_use = function(itemstack, user, pointed_thing)
		local inv = user:get_inventory()
		if not inv:contains_item("main", "lucky_weapons:flare") then
			minetest.sound_play("shooter_click", {object=user})
			return itemstack
		end
		if not minetest.setting_getbool("creative_mode") then
			inv:remove_item("main", "lucky_weapons:flare 1")
		...
		if pos and dir and yaw then
			pos.y = pos.y + 1.5
			local obj = minetest.add_entity(pos, "lucky_weapons:flare_entity")
			if obj then
...
if SHOOTER_ENABLE_CRAFTING == true then
	minetest.register_craft({
		output = "lucky_weapons:flare",
		recipe = {
			{"tnt:gunpowder", "wool:red"},
		},
	})
	minetest.register_craft({
		output = "lucky_weapons:flaregun",
		recipe = {
