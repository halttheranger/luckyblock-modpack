local function throw_hook(itemstack, user, vel)
	...
	if pos and dir and yaw then
		...
		pos.y = pos.y + 1.5
		local obj = minetest.add_entity(pos, "lucky_weapons:hook")
		if obj then
...
minetest.register_entity("lucky_weapons:hook", {
	physical = true,
	timer = 0,
	visual = "wielditem",
	visual_size = {x=1/2, y=1/2},
	textures = {"lucky_weapons:grapple_hook"},
	player = nil,
	itemstack = "",
...
minetest.register_tool("lucky_weapons:grapple_hook", {
...
minetest.register_tool("lucky_weapons:grapple_gun", {
	description = "Grappling Gun",
	inventory_image = "shooter_hook_gun.png",
	on_use = function(itemstack, user, pointed_thing)
		local inv = user:get_inventory()
		if inv:contains_item("main", "lucky_weapons:grapple_hook") and 
				inv:contains_item("main", "tnt:gunpowder") then
			inv:remove_item("main", "tnt:gunpowder")
			minetest.sound_play("shooter_reload", {object=user})
			local stack = inv:remove_item("main", "lucky_weapons:grapple_hook")
			itemstack = "lucky_weapons:grapple_gun_loaded 1 "..stack:get_wear()
		else
...
minetest.register_tool("lucky_weapons:grapple_gun_loaded", {
	description = "Grappling Gun",
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		itemstack = ItemStack("lucky_weapons:grapple_hook 1 "..itemstack:get_wear())
		throw_hook(itemstack, user, 20)
		return "lucky_weapons:grapple_gun"
	end,
})

if SHOOTER_ENABLE_CRAFTING == true then
	minetest.register_craft({
		output = "lucky_weapons:grapple_hook",
		recipe = {
	...
	minetest.register_craft({
		output = "lucky_weapons:grapple_gun",
		recipe = {
			{"", "default:steel_ingot", "default:steel_ingot"},
			{"", "", "default:diamond"},
		},
