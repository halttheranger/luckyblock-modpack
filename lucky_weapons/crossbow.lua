...
minetest.register_alias("lucky_weapons:arrow", "lucky_weapons:arrow_white")
minetest.register_alias("lucky_weapons:crossbow_loaded", "lucky_weapons:crossbow_loaded_white")
...
minetest.register_entity("lucky_weapons:arrow_entity", {
	physical = false,
	visual = "mesh",
...
for _, color in pairs(dye_basecolors) do
	minetest.register_craftitem("lucky_weapons:arrow_"..color, {
		description = color:gsub("%a", string.upper, 1).." Arrow",
		inventory_image = get_texture("arrow_inv", color),
	})
	minetest.register_tool("lucky_weapons:crossbow_loaded_"..color, {
		...
		on_use = function(itemstack, user, pointed_thing)
			...
			itemstack = "lucky_weapons:crossbow 1 "..itemstack:get_wear()
			local pos = user:getpos()
			local dir = user:get_look_dir()
			local yaw = user:get_look_yaw()
			if pos and dir and yaw then
				pos.y = pos.y + 1.5
				local obj = minetest.add_entity(pos, "lucky_weapons:arrow_entity")
				local ent = nil
				if obj then
...
minetest.register_tool("lucky_weapons:crossbow", {
	description = "Crossbow",
	inventory_image = "shooter_crossbow.png",
	on_use = function(itemstack, user, pointed_thing)
		local inv = user:get_inventory()
		local stack = inv:get_stack("main", user:get_wield_index() + 1)
		local color = string.match(stack:get_name(), "lucky_weapons:arrow_(%a+)")
		if color then
			minetest.sound_play("shooter_reload", {object=user})
			if not minetest.setting_getbool("creative_mode") then
				inv:remove_item("main", "lucky_weapons:arrow_"..color.." 1")
			end
			return "lucky_weapons:crossbow_loaded_"..color.." 1 "..itemstack:get_wear()
		end
		for _, color in pairs(dye_basecolors) do
			if inv:contains_item("main", "lucky_weapons:arrow_"..color) then
				minetest.sound_play("shooter_reload", {object=user})
				if not minetest.setting_getbool("creative_mode") then
					inv:remove_item("main", "lucky_weapons:arrow_"..color.." 1")
				end
...
if SHOOTER_ENABLE_CRAFTING == true then
	...
	minetest.register_craft({
		output = "lucky_weapons:arrow_white",
		recipe = {
			{"default:steel_ingot", "", ""},
	...
	for _, color in pairs(dye_basecolors) do
		if color ~= "white" then
			minetest.register_craft({
				output = "lucky_weapons:arrow_"..color,
				recipe = {
					{"", "dye:"..color, "lucky_weapons:arrow_white"},
				},
			})
		end
	end
